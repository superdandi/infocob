...
const statusConfig = {
  active: { dot: "bg-success", hover: "hover:border-success/30" },
  archived: { dot: "bg-text-muted", hover: "hover:border-text-muted/30" },
  unknown: { dot: "bg-text-muted/40", hover: "hover:border-text-muted/20" },
};
  ...
  return (
    <div>
      <div className="grid grid-cols-3 gap-4 mb-10 max-w-md mx-auto">
        {[
          { label: t("portfolio.en-linea"), count: activeCount, color: "text-success" },
          { label: t("portfolio.archivados"), count: archivedCount, color: "text-text-muted" },
          { label: t("portfolio.recuperando"), count: unknownCount, color: "text-text-muted/60" },
        ].map((s) => (
      ...
      <div className="relative max-w-md mx-auto mb-6">
        ...
        <input
          type="text"
          placeholder={t("portfolio.buscar")}
          value={search}
      ...
      <div className="flex flex-wrap justify-center gap-2 mb-10">
        ...
        >
          {t("portfolio.todos", { count: projects.length })}
        </button>
        {categories.map((cat) => (
          <button
            key={cat.value}
      ...
      {filtered.length === 0 ? (
        <div className="text-center py-16">
          ...
          <p className="text-text-muted">{t("portfolio.no-resultados")}</p>
        </div>
      ) : (
